...
import { BreederCentralImage, type ImageCenter } from "./BreederCentralImage";

const IMAGE_URL = "https://example.com/photo.jpg";

describe("BreederCentralImage", () => {
  it("renders an img with the imageUrl and alt", () => {
    render(<BreederCentralImage imageUrl={IMAGE_URL} alt="A photo" />);
  ...
  it("applies a custom focal point", () => {
    render(<BreederCentralImage imageUrl={IMAGE_URL} alt="A photo" center={[0, 100]} />);
    const img = screen.getByRole("img", { name: "A photo" });
  ...
  it("clamps out-of-range focal point values", () => {
    ...
    render(<BreederCentralImage imageUrl={IMAGE_URL} alt="A photo" center={outOfRange} />);
    const img = screen.getByRole("img", { name: "A photo" });
    expect(img).toHaveStyle({ objectPosition: "0% 100%" });
  });

  it("applies the objectFit prop", () => {
    render(<BreederCentralImage imageUrl={IMAGE_URL} alt="A photo" objectFit="contain" />);
    const img = screen.getByRole("img", { name: "A photo" });
  ...
  it("passes className and style to the container", () => {
    const { container } = render(
      <BreederCentralImage imageUrl={IMAGE_URL} alt="A photo" className="hero" style={{ width: "50%" }} />,
    );
